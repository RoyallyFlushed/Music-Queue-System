...
game.Players.PlayerAdded:connect(function(player)
    player.Chatted:connect(function(message)
		...
		elseif (message:lower()):sub(1,5) == ':skip' then
			SkipCurrentSongToggle = (#CurrentPlaylist > 0 or #RepeatPlaylist > 0) and true or false
		...
		elseif (message:lower()):sub(1,10) == ':normalise' then
			...
			Filter()
		elseif (message:lower()):sub(1,5) == ':stop' then
			ResetPlaylist = true
			RepeatPlaylistToggle = false
			RepeatToggleWarden = false
			Filter()
